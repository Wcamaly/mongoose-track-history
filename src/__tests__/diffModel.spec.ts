...
import { IPostDoc, Post } from '../__fixtures__/Post';
...
describe('diff', () => {
  ...
  it('revertToVersion()', async () => {
    ...
    post.title = 'updated';
    post.subjects = [{ name: 'math', _id: new mongoose.Types.ObjectId().toHexString() }, { name: 'air', _id: new mongoose.Types.ObjectId().toHexString() }];
    await post.save();
    ...
    post2.subjects = [{ name: 'math2', _id: new mongoose.Types.ObjectId().toHexString() }, { name: 'air2', _id: new mongoose.Types.ObjectId().toHexString() }];
    await post2.save();
